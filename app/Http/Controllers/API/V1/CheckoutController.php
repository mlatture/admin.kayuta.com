...
public function checkout(Request $request)
{
    ...
    public function modificationReservations(Request $request)
    {
        ...
        try {
            ...
            // 1. Load Draft
            $draft = \App\Model\ReservationDraft::where('draft_id', $v['draft_id'])->lockForUpdate()->first();
            if (!$draft) { DB::rollBack(); return response()->json(['ok'=>false, 'message'=>'Draft not found'], 404); }
            if ((int)$draft->customer_id !== (int)$v['customer_id']) { DB::rollBack(); return response()->json(['ok'=>false, 'message'=>'Unauthorized'], 403); }
            if ($draft->status !== 'draft') { DB::rollBack(); return response()->json(['ok'=>false, 'message'=>'Draft finalized'], 409); }

            $cartData = is_string($draft->cart_data) ? json_decode($draft->cart_data, true) : (array)$draft->cart_data;
            if (empty($cartData)) { DB::rollBack(); return response()->json(['ok'=>false, 'message'=>'Empty cart'], 400); }

            $originalIds = is_string($draft->original_reservation_ids) ? json_decode($draft->original_reservation_ids, true) : (array)$draft->original_reservation_ids;
            $originalIds = array_filter((array)$originalIds);
            
            $originalReservations = \App\Model\Reservation::whereIn('id', $originalIds)->lockForUpdate()->get();
            $customer = \App\Model\User::findOrFail($draft->customer_id);

            // 2. Identify Changes
            $originalMap = [];
            $originalTotalPaid = 0.0;
            foreach ($originalReservations as $r) {
                // Key by siteid for "same site" logic
                $key = (string)$r->siteid; 
                $originalMap[$key] = $r;
                $originalTotalPaid += (float)($r->totalcharges ?? $r->total ?? 0); // Assuming total was paid
            }
            ...
            foreach ($cartData as $item) {
                $siteId = (string)($item['siteid'] ?? $item['site_id'] ?? $item['id'] ?? '');
                if (!$siteId) continue;
                
                $base = (float)($item['base'] ?? 0);
                $lock = (float)($item['lock_fee_amount'] ?? $item['site_lock_fee'] ?? 0);
                $total = (float)($item['grand_total'] ?? $item['total'] ?? ($base + $lock));
                
                $newGrandTotal += $total;
                $processedSiteIds[] = $siteId;

                if (isset($originalMap[$siteId])) {
                     $itemsToUpdate[] = ['r' => $originalMap[$siteId], 'item' => $item, 'total' => $total, 'base' => $base, 'lock' => $lock];
                } else {
                     $itemsToCreate[] = ['item' => $item, 'total' => $total, 'base' => $base, 'lock' => $lock];
                }
            }

            $reservationsToCancel = [];
            foreach ($originalMap as $sid => $r) {
                if (!in_array($sid, $processedSiteIds)) {
                    $reservationsToCancel[] = $r;
                }
            }

            // 3. Financials
            $netDelta = $round2($newGrandTotal - $originalTotalPaid);
            ...
            if ($netDelta > 0) {
                 ...
                 $amount = $netDelta;
                 $xRef = null;
                 $label = ucfirst($v['payment_method']);
                 ...
                 if ($v['payment_method'] === 'card') {
                     ...
                     $response = Http::asForm()->post('https://x1.cardknox.com/gateway', $post);
                     
                     if (!$response->successful()) throw new \Exception('Gateway communication error');
                     
                     $resp = [];
                     parse_str($response->body(), $resp);
                     
                     if (($resp['xStatus']??'') !== 'Approved') throw new \Exception($resp['xError'] ?? 'Payment Failed');
                     
                     $xRef = $resp['xRefNum'];
                     $label = $resp['xCardType'] ?? 'Card';
                 }
                 
                 $baseCartId = $originalReservations->first() ? $originalReservations->first()->cartid : ('MOD-'.$draft->draft_id);
                ...
                // REFUND
                $amount = abs($netDelta);
                $origPayId = $originalReservations->first()->payment_id ?? null;
                $origPay = $origPayId ? \App\Model\Payment::find($origPayId) : null;
                $origRef = $origPay->x_ref_num ?? null;
                
                if ($origRef && $v['payment_method'] === 'card') {
                     $post = [
                        'xKey' => config('services.cardknox.api_key'),
                        'xCommand' => 'cc:Refund',
                        'xAmount' => $amount,
                ...
                $baseCartId = $originalReservations->first() ? $originalReservations->first()->cartid : ('MOD-'.$draft->draft_id);
                $resId = $originalReservations->first()->id ?? 0;
            ...
            // 4. Apply Updates
            foreach ($itemsToUpdate as $up) {
                $r = $up['r']; $i = $up['item'];
                $r->fill([
                    'cid' => \Carbon\Carbon::parse($i['start_date'])->format('Y-m-d 15:00:00'),
                    'cod' => \Carbon\Carbon::parse($i['end_date'])->format('Y-m-d 11:00:00'),
                    'nights' => \Carbon\Carbon::parse($i['end_date'])->diffInDays(\Carbon\Carbon::parse($i['start_date'])),
                    'total' => $up['total'],
                    'totalcharges' => $up['total'],
                    'subtotal' => $up['base'],
                    'sitelock' => $up['lock']
                ]);
                $r->save();
            ...
            foreach ($reservationsToCancel as $r) {
                $r->status = 'Cancelled';
                $r->cancel_reason = 'Modification Update';
                $r->save();
            }

            foreach ($itemsToCreate as $cr) {
                 $i = $cr['item'];
                 $baseCartId = $originalReservations->first() ? $originalReservations->first()->cartid : ('MOD-'.$draft->draft_id);

                 \App\Model\Reservation::create([
                    'cartid' => $baseCartId,
                    'customernumber' => $customer->id,
                    'siteid' => $i['siteid'] ?? $i['id'],
                    'cid' => \Carbon\Carbon::parse($i['start_date'])->format('Y-m-d 15:00:00'),
                    'cod' => \Carbon\Carbon::parse($i['end_date'])->format('Y-m-d 11:00:00'),
                    'total' => $cr['total'],
                    'totalcharges' => $cr['total'],
                    'subtotal' => $cr['base'],
                    'sitelock' => $cr['lock'],
                    'nights' => \Carbon\Carbon::parse($i['end_date'])->diffInDays(\Carbon\Carbon::parse($i['start_date'])),
            ...
            return response()->json([
                ...
                'net_delta' => $netDelta,
                'ref' => $paymentId ?? $refundId
            ]);

        } catch (\Throwable $e) {
            DB::rollBack();
            return response()->json(['ok'=>false, 'message'=>$e->getMessage(), 'line'=>$e->getLine()], 500);
        }
    }
